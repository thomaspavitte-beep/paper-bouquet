...
import { generateArrangement, type ArrangeMode } from "./arrange";
...
export function exportSvg(lib: Library, p: BouquetParams): string {
  ...
  const arr = generateArrangement(mulberry32(p.bloomSeed), palette, lib, {
    ...
    width: vase.width,
    height: vase.height,
  }, { mediums: p.mediums, density: p.density, curviness: p.curviness, mode: p.mode, literal: true });

  const view = p.mode === "stems" ? VIEW : arr.bounds ? { x: arr.bounds.x, y: arr.bounds.y, w: arr.bounds.w, h: arr.bounds.h } : VIEW;
  return (
